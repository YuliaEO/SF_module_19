...
class PetFriends:
    ...
    def get_app_key(self, email: str, password: str) -> json:
        '''Метод делает get запрос к API  с емейлом и паролем
         и возвращает код статуса запроса и ключ в формате json либо в виде строки'''
        headers = {
            'email': email,
            'password': password
        }
    ...
    def get_list_of_pets(self, auth_key: json, filter: str) -> json:
        '''Метод делает get запрос к API сервера с ключом в headers и
    ...
    def add_new_pets(self, auth_key: json, name: str, animal_type: str, age: str, pet_photo: str) -> json:
        ...
        возвращает код статуса запроса результат в формате json с информацией о питомце. '''
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) ->json:
        ...
        pet_id питомца. Возвращает код статуса запроса и результат в формате json с информацией о питомце.'''
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
